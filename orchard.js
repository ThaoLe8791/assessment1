...
// CODE HERE
let totalAcres = 0;
for (const eachDay of fujiAcres){ // loop through each element in each array and then add it to totalAcres
    totalAcres += eachDay
}
for (const eachDay of galaAcres){
    totalAcres += eachDay
}
for (const eachDay of pinkAcres){
    totalAcres += eachDay
}
...
let averageDailyAcres = totalAcres/7; // total/7daysaweek=average daily
...
let galaTons = [];
let pinkTons = [];
for (let i=0; i< fujiAcres.length; i++){ //for each element (acres in each day), multiply by 6.5 to calculate amount of apples for the corresponding acres, then add the value to array
...
let sumGala = 0;
let sumPink = 0;
for (let tonEachDay of fujiTons){ //tonEachday * 2000  = pounds per day; add them up for each iteration to get the sum of each variety
    sumFuji+= tonEachDay *2000
}
for (let tonEachDay of galaTons){
    sumGala+= tonEachDay *2000
}
for (let tonEachDay of pinkTons){
    sumPink+= tonEachDay *2000
}
console.log('Fuji', sumFuji)
console.log('Gala', sumGala)
console.log('Pink', sumPink)
...
// CODE HERE
let fujiProfit = sumFuji * fujiPrice; // profit = totalpounds * unit price
let galaProfit = sumGala * galaPrice;
let pinkProfit = sumPink * pinkPrice;
...
// CODE HERE
let totalProfit = fujiProfit + galaProfit + pinkProfit; //add total of 3 units
